...
T_MAX = int(os.environ.get("RWKV_T_MAX", 2048))  # TAKES LOTS OF VRAM!
# it's possible to go beyond CUDA limitations if you slice the ctx and pass the hidden state in each slice

from torch.utils.cpp_extension import load

wkv_cuda = load(name=f"wkv_{T_MAX}", sources=[
    os.path.join(os.path.split(__file__)[0], "../", "cuda/wkv_op.cpp"), 
    os.path.join(os.path.split(__file__)[0], "../", "cuda/wkv_cuda.cu")
    ], verbose=True, extra_cuda_cflags=["-res-usage", "--maxrregcount 60", "--use_fast_math", "-O3", "-Xptxas -O3", f"-DTmax={T_MAX}"],
    extra_include_paths="/home/surya/miniconda3/lib")
